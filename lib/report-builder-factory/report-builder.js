...
module.exports = class ReportBuilder {
    ...
    prepareTestResult(test) {
        test = this._TestAdapter.create(test, this._toolConfig).prepareTestResult();
        const {name, suitePath, browserId, imagePath = ''} = test;
        const nodeResult = findNode(this.getSuites(), suitePath);
