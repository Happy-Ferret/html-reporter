...
module.exports = class ToolRunner {
    ...
    updateReferenceImage(tests) {
        ...
        return Promise.map(tests, (test) => {
            test = this._prepareUpdateResult(test);
            reportBuilder.addUpdated(test);

            return reporterHelper.updateReferenceImage(reportBuilder.format(test), this._reportPath)
                .then(() => {
                    this._tool.emit(this._tool.events.UPDATE_RESULT, test);
                    const formattedResult = reportBuilder.format(test);

                    return prepareTestResult(reportBuilder.getSuites(), formattedResult.prepareTestResult());
                });
        });
    }
